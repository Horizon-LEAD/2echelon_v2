...
read_config <- function(path) {
  ...
  config <- c(k, workshift, handlingTimeFirstEchelon,
              handlingTimeSecondEchelon, stopTimeFirstEchelon,
              stopTimeSecondEchelon, distanceType,
              haversineCalibration)
...
read_facility <- function(path, i, handlingTime) {
  ...
  #i = leg
  #---------------------------------------------------------------
  file_facilities <- paste(path, sep = "")
  ...
  facility <- c(facilityUI[i, 1], handlingTime, as.double(facilityUI[i, 6]),
               as.double(facilityUI[i, 7]))
...
get_vehicle_attributes <- function(vehicle) {
  #---------------------------------------------------------------------------
  # convert the string with the attributes of the vehicles to a vector
  # with the attributes
  #---------------------------------------------------------------------------
...
read_deliveryZone <- function(path, file_services, i, is_twoEchelon, facility) {
  #---------------------------------------------------------------------------
  ...
  fdServices <- read_services_ErroHandling(read_services(file_services))
  if (fdServices[1] == 0) {
  ...
  if (is_twoEchelon && i == 1) {
    ...
    if (strtoi(zoneAux[1]) == 1 || strtoi(zoneAux[3] == 1)) {
      if (strtoi(zoneAux[1]) == 1) { # the case of Madrid
        str_url <- zoneAux[2]
        str_file_name <- paste(dirname(path), zoneAux[4], sep = "/")
    ...
    zone <- c(i, zoneAvgOrderSize, zoneArea, zoneCentroidX, zoneCentroidY,
             zoneNOrders)
  }
...
write_outputJSON <- function(dfOutput, fpath) {
  #---------------------------------------------------------------------------
  #write the output in the output file (.json); only the outputs
  #---------------------------------------------------------------------------
  dfOutput_json <- toJSON(dfOutput, pretty = TRUE)
  fd <- write(dfOutput_json, fpath)

  return(fd)
}

twoEchelon_output_to_CO2_Models_outputJSON <- function(
  path, str_file_in, str_file_out, echelon_index
) {
  #--------------------------------------------------------------------
  ...
  n <- nrow(df)
  #access to the first vehicle
  # (if one echelon just one row; if two echelon the first row)
  #it contains the category, fuel, segment and
  # Eurostandard columns separated by semicolon
  vehicle <- df[echelon_index, 12]
...
twoEchelon_output_to_COPERT_outputJSON <- function(
  path, str_file_in, str_file_out, echelon_index
) {
#-----------------------------------------------------------------------------
# This function reads the output produced by the 2Echelon model and creates
# the file input for the wrapper function developed for the LMT optimization
# model
#-----------------------------------------------------------------------------
  file_input <- paste(path, sep = "")
  str_file_in <- paste(file_input, str_file_in, sep = "")
  ...
  # access to the first vehicle
  # (if one echelon just one row; if two echelon the first row)
  # it contains the category, fuel, segment and Eurostandard columns
...
twoEchelon_output_to_EVCO2_outputJSON <- function(
  path, str_file_in, str_file_out, echelon_index
) {
  #---------------------------------------------------------------------------
  # This function reads the output produced by the 2Echelon model and creates
  ...
  #access to the first vehicle
  # (if one echelon just one row; if two echelon the first row)
  # it contains the category, fuel, segment and Eurostandard
...
LL1_update_services <- function(path, file_in) {
  ...
  file_out <- paste(path, sep = "")
  file.copy(file_in, file_out, overwrite = TRUE,
